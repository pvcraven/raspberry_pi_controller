    ...
    if "" != keyboard_event_file:
        try:
            ...
            while going_on and event:
                (seconds, microseconds, eventType, eventCode, value) = struct.unpack(struct_format, event)
                ...
                if 1 == eventType:
                    ...
                    print("Event Size [" + str(event_size) + "] Type [" + str(eventType) + "], code [" +
                          str(eventCode) + "], value [" + str(value) + "] at [" + friendly_dts + "]", flush=True)
                if 1 == eventCode:
